addLayer("q", {
	...
	sf1 = new Decimal(0.01)
	    ...
	    if (exp2.gte("1e25")) exp2 = exp2.div(1e25).pow(sf1).mul(1e25).pow_base(10)
	    if (hasUpgrade('P', 11)) exp2 = exp2.log10().mul(6).pow(1.673).pow_base(10).pow_base(10)
	    if (exp2.gte("1e50")) exp2 = exp2.div(1e50).pow(0.06).mul(1e50).pow_base(10)
	    if (exp2.gte("1e100")) exp2 = exp2.div(1e100).pow(0.02).mul(1e100).pow_base(10)
	    if (exp2.gte("1e200")) exp2 = exp2.log10().div(200).pow(0.5).mul(200).min("1000000").pow_base(10)
	    if (exp2.gte("1")) exp = exp2.pow_base(10)
        return exp
    },
...
addLayer("Qc", {
    ...
    branches: ["q"],
    gainMult() { // Calculate the multiplier for main currency from bonuses
        mult = player.q.points.add(1e43).log10()
        exp = new Decimal(1)
	one = new Decimal(1)
	    ...
	    if (hasAchievement("A",45)) mult = mult.log10().pow(upgradeEffect('g',22)).pow_base(10)
        return mult
    },
    gainExp() { // Calculate the exponent on main currency from bonuses
   	 exp = new Decimal(1)
...
addLayer("SP", {
	...
	if (hasUpgrade('DT', 35)) mult = mult.add(upgradeEffect('DT',35).sub(4).pow_base("1e15000"))
    ...
    row: "side", // Row the layer is in on the tree (0 is the first row)
	 upgrades: {
        ...
        11: {
		    ...
		    if (hasUpgrade('DT', 35)) base = base.pow(upgradeEffect('DT',35).add(1).div(2))
                return base
            },
             effectDisplay() {
